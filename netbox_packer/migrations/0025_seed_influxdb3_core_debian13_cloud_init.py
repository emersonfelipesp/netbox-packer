...
write_files:
  - path: /usr/local/sbin/install-influxdb3-core
    ...
    content: |
      ...
      # Every bound here matters: this runs as the final runcmd entry, so a curl
      # that never returns hangs cloud-init forever and the clone never reaches the
      # bounded readiness diagnostics below. --retry alone does not help — a server
      # that completes TLS and then simply stops sending data produces no error to
      # retry. --max-filesize also stops a hostile endpoint from filling /tmp before
      # the key is ever filtered by fingerprint.
