...
This plugin's own list views used the *empty* dict form, which happens to
survive (iterating it yields nothing) — but that is luck, not correctness, and
it left the whole render path unverified on any NetBox version.
...
_REQUIRE_DJANGO = os.environ.get("NETBOX_PACKER_REQUIRE_DJANGO", "").lower() in (
    "1",
    "true",
    "yes",
)
...
_PLUGIN_MODULE_PREFIX = "netbox_packer"
...
_MINIMUM_EXPECTED_LIST_VIEWS = 3
...
def _registered_plugin_list_views() -> tuple[tuple[str, type[ObjectListView]], ...]:
    ...
    importlib.import_module("netbox_packer.urls")
